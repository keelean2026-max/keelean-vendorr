...
const emailSchema = z.object({
  identifier: z.string().min(1, "Email is required").email("Please enter a valid email address"),
});

const phoneSchema = z.object({
  identifier: z.string().min(1, "Phone number is required").regex(/^\d{10}$/, "Please enter a valid 10-digit phone number"),
});

const otpSchema = z.object({
  otp: z.string().min(6, "OTP must be at least 6 digits").regex(/^\d+$/, "OTP must contain only numbers"),
});
...
const baseButtonClasses =
  ...
  "bg-gradient-to-br from-[#808A93] to-[#95A1AC] " +
...
const VendorAuth = () => {
  ...
  // Dynamic resolver (recomputes when identifierType or otpSent changes)
  const resolver = useMemo(() => zodResolver(createSchema(identifierType, otpSent)), [identifierType, otpSent]);
  ...
  // Keep validation schema and form state in sync when identifier type changes
  useEffect(() => {
    // Reset fields when identifier type changes
    ...
    // trigger(); // optionally trigger validation
  }, [identifierType, reset, clearErrors]);
  ...
  const handleSendOtp = async (data) => {
    try {
      ...
      alert(
        `OTP sent to ${identifierType === "phone" ? `${selectedCountry?.dialCode ?? ""} ${data.identifier}` : data.identifier}`
      );
    } catch (error) {
  ...
  const fullPhoneNumber =
    identifierType === "phone" && selectedCountry ? `${selectedCountry.dialCode} ${identifierValue}` : identifierValue;
  ...
  const backgroundClasses =
    "min-h-screen flex items-center justify-center px-4 bg-gradient-to-br from-slate-50 via-slate-100 to-slate-200";
      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="bg-white/95 rounded-3xl border border-slate-100 shadow-2xl shadow-slate-200 px-8 py-8 md:py-9">
          <div className="flex items-center justify-center gap-3 mb-6">
            ...
            <span className="text-3xl font-semibold tracking-tight text-slate-900">Login</span>
          </div>

          <h1 className="text-[22px] font-semibold text-slate-900 mb-1">{!otpSent ? "Sign in or create account" : "OTP verification"}</h1>

          {otpSent && (
            <p className="text-sm font-medium text-gray-600 mb-7">
              We’ve sent an OTP on {channelLabel} to:
              <br />
              <span className="font-semibold text-gray-700 break-all">{identifierType === "phone" ? fullPhoneNumber : identifierValue}</span>
            </p>
          )}
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="mt-4 space-y-4" noValidate>
            ...
            <div className="flex gap-2 mb-4">
              <button
                ...
                className={`flex-1 py-2.5 px-4 rounded-lg border text-sm font-medium transition-all duration-200 ${
                  identifierType === "email" ? "bg-blue-50 border-blue-500 text-blue-700 shadow-sm" : "bg-white border-slate-300 text-slate-600 hover:bg-slate-50"
                }`}
              >
                Email
              </button>
              <button
                ...
                className={`flex-1 py-2.5 px-4 rounded-lg border text-sm font-medium transition-all duration-200 ${
                  identifierType === "phone" ? "bg-blue-50 border-blue-500 text-blue-700 shadow-sm" : "bg-white border-slate-300 text-slate-600 hover:bg-slate-50"
                }`}
              >
                Mobile
              </button>
            ...
            {/* Input Field */}
            <div>
              <label className="block text-sm font-medium text-slate-800 mb-1.5">{identifierType === "email" ? "Email address" : "Mobile number"}</label>
                ...
                <div>
                  <div className="flex gap-2">
                    ...
                    <div className="relative shrink-0" ref={dropdownRef}>
                      <button
                        type="button"
                        onClick={() => !otpSent && setShowCountryDropdown((s) => !s)}
                        disabled={otpSent || isSubmitting}
                      ...
                      >
                        <span className="flex items-center gap-2">
                          {selectedCountry && <ReactCountryFlag countryCode={selectedCountry.code} svg style={{ width: "20px", height: "15px" }} title={selectedCountry.name} />}
                          ...
                          <div className="overflow-y-auto flex-1">
                            {filteredCountries.length > 0 ? (
                              filteredCountries.map((country) => (
                                ...
                                >
                                  <ReactCountryFlag countryCode={country.code} svg style={{ width: "20px", height: "15px" }} title={country.name} />
            ...
            {otpSent && (
              <div className="space-y-4 border-t border-slate-200 pt-4">
                <div>
                  <label className="block text-sm font-medium text-slate-800 mb-1.5">Enter OTP</label>
                  <input
                    type="text"
                    {...register("otp")}
                    maxLength={6}
                ...
                <button type="submit" disabled={isSubmitting || !otpValue || otpValue.length < 4} className={baseButtonClasses}>
                ...
                <button
                  ...
                  className={`w-full text-xs font-medium py-2.5 rounded-lg border transition ${
                    canResend ? "bg-white hover:bg-slate-50 border-slate-300 text-slate-700" : "bg-slate-100 border-slate-200 text-slate-400 cursor-not-allowed"
                  }`}
                >
            ...
            {!otpSent && (
              <button type="submit" disabled={isSubmitting || !identifierValue} className={baseButtonClasses}>
